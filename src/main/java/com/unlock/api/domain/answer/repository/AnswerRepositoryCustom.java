...
public interface AnswerRepositoryCustom {
     ...
     */
    List<ArchiveSummaryResponse> findMonthlyArchiveSummary(Couple couple, Long userId, Long partnerId, boolean isSubscribed, int year, int month);

    /**
     * 특정 커플의 전체 질문 아카이브를 페이징 처리하여 조회합니다.
     * @param sortDirection "DESC" (최신순) 또는 "ASC" (오래된순)
     */
    List<ArchiveSummaryResponse> findArchiveList(Couple couple, Long userId, Long partnerId, boolean isSubscribed, int page, int size, String sortDirection);
}

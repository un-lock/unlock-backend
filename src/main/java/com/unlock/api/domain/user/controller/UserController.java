...
public class UserController {
    ...
    public ApiCommonResponse<String> updateNickname(
            @Parameter(hidden = true) @CurrentUser Long userId,
            @RequestBody @Valid NicknameUpdateRequest request) {
        return ApiCommonResponse.success("닉네임이 변경되었습니다.", userService.updateNickname(userId, request));
    }

    @Operation(summary = "비밀번호 변경", description = "로그인된 상태에서 현재 비밀번호를 확인한 후 새로운 비밀번호로 변경합니다.")
    @ApiResponse(responseCode = "200", description = "변경 성공")
    @PatchMapping("/me/password")
    public ApiCommonResponse<Void> updatePassword(
            @Parameter(hidden = true) @CurrentUser Long userId,
            @RequestBody @Valid PasswordUpdateRequest request) {
        userService.updatePassword(userId, request);
        return ApiCommonResponse.success("비밀번호가 성공적으로 변경되었습니다.", null);
    }
    ...
    @DeleteMapping("/me")
    public ApiCommonResponse<Void> withdraw(@Parameter(hidden = true) @CurrentUser Long userId) {
        userService.withdraw(userId);
        return ApiCommonResponse.success("회원 탈퇴가 완료되었습니다. 그동안 이용해 주셔서 감사합니다.", null);
    }
}